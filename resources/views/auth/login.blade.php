...
<head>
    ...
    <link class="shortcut icon" href="/images/favicon.png" type="image/x-icon">
    <!-- FontAwesome for Icons -->
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.5.2/css/all.min.css">
    @vite(['resources/css/app.css', 'resources/js/app.js'])
</head>

<body class="bg-brand-cream text-gray-800 font-sans min-h-screen h-screen overflow-hidden w-full flex flex-col md:flex-row selection:bg-brand-pink/20 selection:text-brand-pink">
    ...
    <div class="w-full md:w-1/2 lg:w-[42%] xl:w-[38%] h-full md:h-screen overflow-y-auto bg-white border-r border-slate-100/50 relative z-10 flex flex-col justify-between p-6 sm:p-10 md:p-12 lg:p-16">
        
        <!-- Header (Back to Home) -->
        <div class="w-full mb-8 md:mb-0">
            <a href="/" class="inline-flex items-center gap-2 text-brand-pink hover:text-brand-purple transition-colors group">
        ...
        <div class="my-auto py-4">
            ...
            <form action="{{ route('login') }}" method="POST" class="space-y-4">
                ...
                <!-- Google OAuth Button -->
                <div>
                    <a href="{{ url('auth/google') }}" class="w-full flex items-center justify-center gap-3 px-4 py-2.5 bg-white hover:bg-gray-50 border border-slate-200/50 rounded-2xl text-xs font-semibold text-gray-600 transition-all duration-200 hover:shadow-sm">
                ...
                <div class="space-y-1.5">
                    <div class="flex justify-between items-center">
                        ...
                        <a href="{{ route('password.request') }}" class="text-[10px] font-bold text-brand-pink hover:text-brand-purple transition-colors">Lupa sandi?</a>
                    </div>
                    <div class="relative">
                        <span class="absolute inset-y-0 left-0 pl-4 flex items-center text-gray-400">
                ...
                <div class="flex items-center">
                    <input type="checkbox" id="remember" name="remember" class="w-4 h-4 text-brand-pink bg-gray-50 border-slate-200 rounded focus:ring-brand-pink/50 accent-brand-pink">
                    <label for="remember" class="ml-2.5 text-[11px] text-gray-500 font-medium select-none">Ingat perangkat ini</label>
                </div>

                <!-- Submit Button -->
                <div class="pt-1.5">
                    <button type="submit" class="w-full py-3 px-4 text-xs font-semibold text-white bg-gradient-to-r from-brand-pink to-brand-purple hover:from-brand-pink/90 hover:to-brand-purple/90 transition-all duration-300 rounded-2xl shadow-md hover:shadow-lg focus:outline-none cursor-pointer text-center">
                        Masuk Sekarang
                    </button>
                </div>
            </form>
        ...
        <div class="w-full pt-6 border-t border-gray-100 text-center">
            <p class="text-[11px] text-gray-500 font-medium">
                Belum memiliki akun? 
                <a href="{{ route('register') }}" class="font-bold text-brand-pink hover:text-brand-purple transition-colors ml-1">Daftar Akun Baru</a>
            </p>
        </div>
    </div>
    ...
    <div class="hidden md:block md:w-1/2 lg:w-[58%] xl:w-[62%] h-screen overflow-hidden relative z-10">
        ...
        <!-- Soft Brand Overlay -->
        <div class="absolute inset-0 bg-gradient-to-br from-brand-purple/45 to-brand-pink/45 flex flex-col justify-between p-12 lg:p-16 text-white">
            <!-- Decorative Top Sparkle (FontAwesome replacement) -->
